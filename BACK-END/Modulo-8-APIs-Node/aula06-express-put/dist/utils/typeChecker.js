...
function checkData(res, data) {
    if (typeof data.productName !== 'string' ||
        ...
        return res
            .status(400)
            .send(`${data.productCost} inválido! O dado esperado em productCost é um número.`);
    }
    if (!Array.isArray(data.productTags) ||
        !data.productTags.every((tag) => typeof tag === 'string')) {
        return res
            .status(400)
            .send(`${data.productTags} inválido! O dado esperado em productTags são textos.`);
    }
    else if (!Array.isArray(data.productRelated) ||
        !data.productRelated.every((related) => typeof related === 'number')) {
        return res
            .status(400)
            .send(`${data.productRelated} inválido! O dado esperado em productRelated são números.`);
    }
}
